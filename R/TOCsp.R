.TOCsp <- function(index, boolean, mask=NULL, nthres=NULL, thres=NULL, NAval=0, P=NA, Q=NA, progress=TRUE, sort='DECREASE', thresorder=TRUE, ordinal=FALSE, FirstThres=NULL, LastThres=NULL, Increment=NULL) {

  #ifelse(!is.null(nthres) & !is.null(thres),stop("Enter nthres OR thres as input to define thresholds, not both at the same time. Or just sort the thresholds decreasing"),ifels())
  ...
  tocd <- .TOCnosp(indval, boolval, mask=mask, nthres=nthres, thres=thres, NAval=NAval, progress=progress,
                   ones.bool=ones.bool, zeros.bool=zeros.bool, population=population, units=units, sort=sort, thresorder=thresorder, ordinal=ordinal, FirstThres=FirstThres, LastThres=LastThres, Increment=Increment)
