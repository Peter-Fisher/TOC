.plotTOC <- function(object, labelThres=FALSE, modelLeg="Model", digits=3, nticks=5, digitsL=1, posL = NULL, offsetL = 0.5, addAUC = TRUE, digitsAUC=2, AUClableX = 0.6, AUClableY = 0.1, addCC = FALSE, ...){
...
units <- object@units
AUC <- object@AUC
CorrectCorner <- object@CorrectCorner
...
if(addAUC){text(AUClableX * population, AUClableY*prevalence * population,paste("AUC: ",round(as.numeric(AUC), digitsAUC)),adj = c(0,1))}
